    ...
    <div className="grid grid-cols-3 gap-2 h-full overflow-hidden">
      {sections.map((s, i) => (
        <div key={i} className="glass-card p-3 overflow-y-auto min-h-0">
          <h3 className="text-base font-bold text-[var(--neon-cyan)] mb-1 sticky top-0 bg-[rgba(10,10,15,0.95)] pb-0.5 z-10">{s.title}</h3>
          <div className="text-[15px] leading-[1.3] text-[var(--text-primary)]"
...
function formatContent(text: string): string {
  return text
    .replace(/^### (.+)$/gm, '<div class="text-[var(--neon-yellow)] font-bold mt-1 text-[15px]">$1</div>')
